...
def tes_on_subset(dataset, cnn, n, pred_vectors, all_label,
                   consider_trains):
    top = [1, 2, 5, 10, 20]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cnn',default='save/resnet-fit/epoch-1.pth')
    parser.add_argument('--pred',default='save/gcn-dense-att/epoch-3000.pred')

    parser.add_argument('--test-set',default='3-hops')#在平台训练好之后，切换到本地跑
    ...
    need_delete=json.load((open('materials/need_delet_3_hops.json')))
    delete_element=need_delete['delet']
    ...
    imagenet_path = '../../../public/ImageNet-all'
    ...
    if TEST_TRAIN:
        for i, wnid in enumerate(train_wnids, 1):
            ...
            hits, tot = tes_on_subset(subset, cnn, n, pred_vectors, i - 1,
        ...
        for i, wnid in enumerate(test_wnids, 1):
            subset = dataset.get_subset(wnid)
            hits, tot = tes_on_subset(subset, cnn, n, pred_vectors, n + i - 1,
                                       consider_trains=args.consider_trains)
            results[wnid] = (hits / tot).tolist()
